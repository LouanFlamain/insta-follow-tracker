...
import { waitForClassWithTimeout } from "./actions/waitforclass.js";
import { Write } from "./actions/write.js";

export const Execute = async (driver: any, data: UserInfoType) => {
  let list = {
    follow: [],
    followers: [],
  };
  ...
  if (list.follow.length === list.followers.length) {
    ...
    let resultFollowers = list.follow.filter(
      (item) => list.followers.indexOf(item) == -1
    );
    let resultFollow = list.followers.filter(
      (item) => list.follow.indexOf(item) == -1
    );

    console.log(
      "followers manquant : ",
      resultFollowers,
      "comptes non suivie en retour : ",
      resultFollow
    );
...
const Clicker = async (driver: any, location: string) => {
...
const Writer = async (driver: any, location: string, data: UserInfoType) => {
  let dataType: any = data[location]; // Utiliser directement les clés du type UserInfoType
...
const Redirection = async (driver: any, location: any) => {
